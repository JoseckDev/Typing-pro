...
class UserProfile:
    ...
    def update_profile(self, wpm: float, accuracy: float, difficulty: str,
        ...
        if not isinstance(profile.get("languages_tried"), set):
            # Convert string/list to set
            if isinstance(profile.get("languages_tried"), str):
                profile["languages_tried"] = {profile["languages_tried"]}
            elif isinstance(profile.get("languages_tried"), list):
                profile["languages_tried"] = set(profile["languages_tried"])
            else:
...
class PracticeMode(ctk.CTkToplevel):
    ...
    def _on_typing(self, event=None):
        """Handle typing and highlight correctness in real-time."""
        typed = self.textbox.get("1.0", "end-1c")  # everything user typed
        target = self.current_text                 # expected text
        ...
        for i, char in enumerate(target):
            ...
            if i < len(typed):
                if typed[i] == char:
                    self.textbox.tag_add("correct", start, end)
                else:
                    self.textbox.tag_add("wrong", start, end)
            else:
                self.textbox.tag_add("pending", start, end)

        # Optional: detect completion
        if typed == target:
            print("✅ Completed typing correctly!")  
            # you could also trigger next round or scoring here
...
class StatsDashboard(tk.Toplevel):
    def __init__(self, parent, profile):
        ...
        stats_text = f"""
WPM: {profile.get('wpm', 'N/A')}
Accuracy: {profile.get('accuracy', 'N/A')}
Languages Tried: {', '.join(profile.get('languages_tried', []))}
"""
        tk.Label(self, text=stats_text, justify="left", font=("Consolas", 14)).pack(padx=20, pady=20)
...
class TypingMasterPro(ctk.CTk):
    ...
    def reset_typing_area(self) -> None:
        ...
        self._apply_pending_tags()
        self.current_wpm = 0.0
        self.profile["wpm"] = self.current_wpm
        self.current_accuracy = 0.0
        self.profile["accuracy"] = self.current_accuracy
        self.stats_label.configure(text="WPM: 0.00 | Accuracy: 0.00%")
        self.time_left = self.round_seconds
        self.timer_label.configure(text=f"Time left: {self.time_left}s")
        self.progress.set(1)
...
# -------------------------------
    ...
    def show_global_impact(self):
        """Show a simple popup with global typing impact info."""
        messagebox.showinfo(
            "Global Impact",
            "Typing skills empower communication, productivity, and digital inclusion worldwide.\n\n"
            "🌍 Every keystroke contributes to global knowledge and progress!"
        )

    def start_training(self):
        """Start a basic training session."""
        messagebox.showinfo("Training", "🚀 Training session started! Focus on accuracy and speed.")

    def start_practice(self):
        """Start a practice mode."""
        weak_keys = getattr(self, "user_profile", None)
        if weak_keys and hasattr(weak_keys, "get_weak_keys"):
            wk = weak_keys.get_weak_keys()
            if wk:
                try:
                    PracticeMode(self, wk, self.user_profile)
                    return
                except Exception:
                    pass
        messagebox.showinfo("Practice", "No weak keys detected yet. Keep typing to generate data!")

    def text_display(self):
        """Display a random sample text for typing."""
        try:
            sample_text = random.choice(SENTENCES.get("English", {}).get("Easy", ["The quick brown fox jumps over the lazy dog."]))
        except Exception:
            sample_text = "The quick brown fox jumps over the lazy dog."
        messagebox.showinfo("Sample Text", f"📖 Try typing this:\n\n{sample_text}")
